...
plot_temp_trend_sample <- function(m_list) {
  ...
  for (m_name in m_list) {
   ...
   sdat$pref <- paste0(str_to_title(gsub("_", " ", sdat$pref)), " (samples = ", n_es, ")") # add number of ess for each pref
  ...
  if (sum(grepl("risk", tolower(unique(dat$pref)))) == 1) {
    ...
    df_text$pref <- factor(df_text$pref, levels=c(risk_lab,time_lab,social_lab))
  ...
  p <- ggplot() +
    ...
    geom_jitter(dat = dat, aes(y = n_incl_es, x = year_of_publication, color = pref),
                height = 0, width = .25, alpha = .3, size = 1.75)+
   geom_smooth(dat = dat, aes(y = n_incl_es, x = year_of_publication, color = pref), method = "lm", formula = y ~x) +
   geom_text(data = df_text, aes(x = x, y = y, label = txt_val), family = "Arial", fontface = "italic", size = 3) +
   ...
   scale_y_log10() +
    scale_x_continuous(breaks = seq(199,2020,5), expand = c(0,0), limits = c(1993,2023)) +
