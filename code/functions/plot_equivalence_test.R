...
#' (at the preference level) compared to a set of low and high equivalence bounds
...
#' @examples
#' plot_equivalence_test(m_file_list =  c("output/risk/mlma_risk_experience.rds",
...
plot_equivalence_test <- function(m_file_list, low_eq, high_eq) {
  ...
  for (curr_file in m_file_list) {
    ...
    eq_test <- read_rds(list.files(path = "output", pattern = paste0("equivalence_test_", slab, ".rds"), 
  ...
  if(length(m_file_list) > 3){
    df$slab = factor(df$slab, levels= rev(str_to_title(c('risk','time','social', 'effort'))))}
  
  if(length(m_file_list) < 4){
