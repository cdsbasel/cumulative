...
author: "Alexandra Bagaïni, Yunrui Liu, Arzie Bajrami, Gayoung Son, Loreen Tisdall, Rui Mata"
date: "8/1/2022"
output:
  html_document:
    theme: cerulean
...
This document showcases step-by-step the analyses described in the study *Age Differences in Economic Preferences: Cumulative meta-analyses of risk, time, social, and effort preferences*. All the functions that are sourced in this script can be found in the "code/functions/" folder. 

###  Required packages
```{r, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
# LOADING ALL REQUIRED PACKAGES (FOR SCRIPTS + FUNCTIONS + PLOTTING)
library(tidyverse) # opening/saving files and data wrangling and plotting
...
preferences <- c("risk", "time", "social", "effort")
...
dat <- bind_rows(read_csv("data/summary/risk/effect_sizes_risk.csv",
                 ...
                 read_csv("data/summary/social/effect_sizes_social.csv",
                          col_types = cols()),
                 read_csv("data/summary/effort/effect_sizes_effort.csv",
                          col_types = cols()))
...
table1 <- bind_rows(
  ...
  table_es_pref_overview(dat = read_csv("data/summary/time/effect_sizes_time.csv",
                                        col_types = cols())),
  table_es_pref_overview(dat = read_csv("data/summary/effort/effect_sizes_effort.csv",
                                        col_types = cols()))
)
table1
```
...
#### Effect Size Risk Overview Table
```{r,eval=TRUE, echo=TRUE, warning=FALSE}
...
### DOMAIN
dat_risk <- dat  %>% mutate(pref = paste0(pref,"_", domain))
bind_rows(
  table_es_pref_overview(dat = filter(dat_risk, pref == "risk_gain")),
  table_es_pref_overview(dat = filter(dat_risk, pref == "risk_loss")),
  table_es_pref_overview(dat = filter(dat_risk, pref == "risk_mixed")))

### TASK TYPE
dat_risk <- dat  %>% mutate(pref = paste0(pref,"_", task_type))
bind_rows(table_es_pref_overview(dat = filter(dat_risk, pref == "risk_experience")),
          table_es_pref_overview(dat = filter(dat_risk, pref == "risk_description")),
)
...
rho = .5
pref = "risk"
dat <- read_csv(sprintf("data/summary/%s/effect_sizes_%s.csv", pref, pref),col_types = cols())
...
##### Additional social analyses

Running the re-analysis of the Sparrow et al. (2021) dataset.

```{r, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
source("code/01_sparrow_ma_reanalysis.R")
```

### Equivalence Testing

To assess whether the effect sizes we obtained are practically or theoretically of *relevance* (i.e., does the overall effect size fall within the boundaries of a smallest effect size of interest or not?) we conducted equivalence tests. We determined the smallest effect size of interest to be |.1|


```{r, eval=TRUE, echo=TRUE, warning=FALSE}
preferences <- c("time", "risk", "social", "effort")
for (pref in preferences) {
  
  m <- read_rds(sprintf("output/%s/mlma_%s.rds", pref, pref))
  ...
  # save output
  write_rds(t, sprintf("output/%s/equivalence_test_%s.rds", pref ,pref))
}
```
...
for (dom in domains) {
  ...
  m <- read_rds(sprintf("output/%s/mlma_%s_%s.rds", pref, pref, dom))
  ...
  # save output
  write_rds(t, sprintf("output/%s/equivalence_test_%s_%s.rds", pref ,pref, dom))
...
for (typ in types) {
  ...
  m <- read_rds(sprintf("output/%s/mlma_%s_%s.rds", pref, pref, typ))
  ...
  # save output
  write_rds(t, sprintf("output/%s/equivalence_test_%s_%s.rds", pref ,pref, typ))
...
For the year-level cumulative meta-analysis, we added the effect size(s) published in one same year after the next, and each time we fit the same three-level model specified above.
```{r, eval=TRUE, echo=TRUE, warning=FALSE}
...
### Meta-Regressions
Here we fit several three-level meta-regression models to investigate the role of different moderators.
```{r, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
...
covariates <- c("incentivization", "domain", "task_type", "cor_type", "study_design", "study_context", "effort_type")

preferences <- c("risk", "social", "time", "effort")
...
for (pref in preferences) {
  ...
  for (covar in covariates) {
    ...
    if (pref != "effort" & covar == "effort_type")
      next
...
for (pref in preferences) {
  ...
  dat <- dat %>% mutate(dec_in_print = (2022 - year_of_publication)/10)
  ...
  temp_m <- rma.mv(yi ~ 1 + dec_in_print,
                   V = V_mat,     
...
for (dom in domains) {
  ...
  sdat <- dat %>% filter(domain == dom) %>% 
    mutate(pref = paste0(pref,"_", dom),
           dec_in_print = (2022 - year_of_publication)/10)
...
for (pref in preferences) {
  ...
  dat_n <- dat_n %>% mutate(dec_in_print = -(2022 - year_of_publication)/10,
                            n_incl_es_log = log10(n_incl_es))
  
  # fit a simple linear regression model
  temp_m <- lm(n_incl_es_log ~ 1 + dec_in_print, data = dat_n)
...
We followed the approach by Koricheva et al. (2013) and compared the effect size and variance of the first study (i.e., Green 1994) with the mean effect size and variance of the rest of the studies.
```{r, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
# get data
...
### Proteus Phenomenon: Effort Discounting
We followed the approach by Koricheva et al. (2013) and compared the effect size and variance of the first article (Westbrook et at., 2013) with the mean effect size and variance of the rest of the studies.
```{r, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
# get data
...
westbrook_dat <- dat %>% filter(grepl("Westbrook", study_label)) # first study
other_dat <- dat %>% filter(!grepl("Westbrook", study_label)) # other studies
...
# calculate z value
z_val <- (m_oth$b[1] - m_westbrook$b[1])/sqrt(m_westbrook$se[1] + m_oth$se[1])
z_val
...
### Citations
Using citation data from Google Scholar we analysed the link between an article's yearly citation count with its (aggregated) effect size and (average) sample size.
```{r, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
source("code/01_cit_regression.R")
```
...
source("code/functions/plot_distrib.R")

# Plotting sensitivity plot
source("code/functions/plot_sensitivity_rho.R")

# Plotting citation analysis
source("code/functions/plot_citation_trends.R")

# Plotting equivalence test
source("code/functions/plot_equivalence_test.R")
```
...
cma_p <- plot_year_cma(m_list = c("output/risk/cma_year_risk.rds",
                                  ...
                                  "output/effort/cma_year_effort.rds"))+
...
temp_p <- plot_temp_trend(m_list = c("output/risk/temp_trend_risk.rds",
                                     ...
                                     "output/effort/temp_trend_effort.rds")) +
...
ggsave("figures/es_year.png", plot = p, width = 17, height = 15, units = "cm", dpi = 600)
...
pA <- plot_equivalence_test(m_file_list = c("output/risk/mlma_risk.rds",
                                            "output/effort/mlma_effort.rds",
                                            "output/time/mlma_time.rds",
                                            "output/social/mlma_social.rds"),
                            low_eq = -.1, high_eq = .1)
...
pA <- pA + labs(tag = "A", x = "") + 
  theme(plot.tag = element_text(size = 11, family = "Arial", face = "bold"))

pB <- plot_equivalence_test(m_file_list =  c("output/risk/mlma_risk_gain.rds",
                                             "output/risk/mlma_risk_loss.rds",
                                             "output/risk/mlma_risk_mixed.rds"),
                            low_eq = -.1, high_eq = .1)
...
pC <- plot_equivalence_test(m_file_list =  c("output/risk/mlma_risk_experience.rds",
                                             "output/risk/mlma_risk_description.rds"),
                            low_eq = -.1, high_eq = .1)
...
p <- (pA/pB/pC)  +
  plot_layout(heights = c(.75,.5, .3))


ggsave("figures/equivalence_test.png", plot = p, width = 19, height = 17, units = "cm", dpi = 600)
```
...
### Overview of Effect Sizes & Sample Sizes (Suppl. Figure)
...
pA <- plot_distrib(dat) +   
  labs(x= "Effect Size (cor)", tag = "A") +
  geom_vline(xintercept = 0, linetype = "dotted", color = "grey30")+
  scale_x_continuous(breaks = seq(-1,1,.5), limits = c(-1,1)) +
  theme(plot.tag = element_text(size = 11, family = "Arial", face = "bold"),
        plot.tag.position = c(.1, 1.05),
        plot.margin = margin(r = 5, t = 12))

# sample sizes
dat_n <- table_sample_sizes(dat) %>% 
  mutate(val = n_incl_es)

pB <- plot_distrib(dat_n) +
  labs(x= "Sample Size (log10)", tag = "B") +
  theme( plot.margin = margin(r = 5, t = 12),
         plot.tag.position = c(.1, 1.05),
         plot.tag = element_text(size = 11, family = "Arial", face = "bold"))+
...
ggsave("figures/es_sample_overview.png", plot = p, width = 29, height = 18, units = "cm", dpi = 600)
```
...
cma_p_e <- plot_forestplot_cma(m = read_rds("output/effort/cma_study_effort.rds"))
...
ma_p_e <- plot_forestplot_ma(m = read_rds("output/effort/mlma_effort.rds"))
...
p <- 
  ...
  ((ma_p_e + 
     ...
     (cma_p_e + 
        theme(plot.margin = unit(c(0,0,0,15), "pt"))))+
  plot_layout(heights = c(1.5,1.5, .5, .5))


ggsave("figures/es_study.png", plot = p, width = 30, height = 42, units = "cm", dpi = 600)
...
p <- plot_temp_trend_sample(m_list = c("output/risk/temp_trend_sample_risk.rds",
                                       ...
                                       "output/effort/temp_trend_sample_effort.rds"))

ggsave("figures/temp_trend_sample.png", plot = p, width = 32, height = 10, units = "cm", dpi = 600)
...
e_p <- plot_forestplot_leave1out(m = read_rds("output/effort/leave1out_study_effort.rds"))
...
p <-  (r_p + labs(title = "Risk")+ 
  ...
  (t_p + labs(title = "Time") + theme(plot.margin = unit(c(0,30,0,0), "pt"),
                                      plot.title = element_text(color = "grey15",
                                                                ...
                                                                face = "bold",
                                                                size = 10))) +
  (a_p + labs(title = "Social") + theme(plot.margin = unit(c(0,30,0,0), "pt"),
                                        plot.title = element_text(color = "grey15",
                                                                  family = "Arial",
                                                                  face = "bold",
                                                                  size = 10))) +
  (e_p + labs(title = "Effort")+ 
...
ggsave("figures/leave1out.png", plot = p, width = 15, height = 40, units = "cm", dpi = 600)
...
e_p <- plot_contour_funnelplot(m = read_rds("output/effort/mlma_effort.rds"),
                               ref = 0)
...
e_p <- plot_funnelplot(m = read_rds("output/effort/mlma_effort.rds"))
...
pB <-    (r_p + 
  ...
  (a_p + labs(y = "")+ 
     theme(plot.margin = unit(c(0,30,0,0), "pt"), legend.position = "none")) +
  (e_p + labs(y = ""))+  plot_layout(ncol = 4)


p <- pA/pB

ggsave("figures/funnel_plot.png", plot = p, width = 40, height = 17, units = "cm", dpi = 600)
...
e_p <- plot_pcurve(pcurve = read_rds("output/effort/pcurve_effort.rds"))
...
p <-   (r_p + labs(title = "Risk")+ 
  ...
  (a_p + labs(title = "Social", y = "")+ 
     theme(plot.margin = unit(c(0,30,0,0), "pt"), legend.position = "none")) +
  (e_p + labs(title = "Effort", y = "")) + plot_layout(ncol = 4)





ggsave("figures/pcurve.png", plot = p, width = 42, height = 12, units = "cm", dpi = 600)
...
e_p <- plot_sensitivity_rho(dat = read_rds("output/effort/mlma_sensitivity_rho_effort.rds"))
# merge plots
...
ggsave("figures/sensitivty_rho.png", plot = p, width = 32, height = 7, units = "cm", dpi = 600)
```


### Citations (Suppl. Figure)

```{r, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
dat <- read_csv(file = "citation/es_cit_dat.csv")

p <- plot_citation_trends(data = dat)

ggsave("citation/citation_trends.png", plot = p, width = 24, height = 20, units = "cm", dpi = 600)
```
...
table_egger(egger = read_rds("output/effort/egger_effort.rds")) %>% 
  write_csv("output/effort/egger_effort_format.csv")
...
table_pcurve(pcurve = read_rds("output/effort/pcurve_effort.rds")) %>% 
  write_csv("output/effort/pcurve_effort_format.csv")
