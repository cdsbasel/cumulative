---
title: "Analysis Workflow of Cumulative Meta-Analyis of Age Differences in Economic Preferences"
...
This document showcases step-by-step the analyses described in the study *Cumulative Meta-Analyis of Age Differences in Economic Preferences*. All the functions that are sourced in this script can be found in the "code/functions/" folder. 
...
# 4B. Using output from 3A to detect selective reporting (i.e., Egger's Test)
source("code/functions/conduct_egger_t.R")

# 4C. Using output from 3A to conduct the CMA
source("code/functions/conduct_cma.R") # by study
source("code/functions/conduct_cma_year.R") # by year

# 4D. Using output from 3A to conduct the leave-one-out analysis
...
To check for publication bias, we run an Egger's regression test
...
for (pref in preferences) {
  ...
  # *precision* metric used can have an effect of whether we get a non-zero slope or not
  egger <- conduct_egger_t(dat = dat, rho = rho)
  egger <- robust(egger, cluster = study_id, clubSandwich = TRUE)
  
  # save output
  write_rds(egger, sprintf("output/%s/egger_%s.rds", pref, pref))
}
```
...
```{r, eval=FALSE, echo=TRUE, warning=FALSE}
rho = .3 # set correlation for variances 
...
### Plot Effect Sizes By Year (Figure 1)
```{r, eval=FALSE, echo=TRUE, warning=FALSE, message=FALSE}
# cma plots
...
ggsave("figures/es_year.png", plot = p, width = 25, height = 15, units = "cm", dpi = 600)
```
...
```{r, eval=FALSE, echo=TRUE, warning=FALSE, fig.height= 20, fig.width=12}
...
p <- ((ma_p_a + 
  ...
  ((ma_p_t + 
     ...
     (cma_p_t + 
        theme(plot.margin = unit(c(0,0,0,15), "pt")))) +
  plot_layout(heights = c(.5, 1.25,1.25))
...
```{r, eval=FALSE, echo=TRUE, warning=FALSE, fig.height= 20, fig.width= 7}
# cma plots
  ...
  (a_p + labs(title = "Social")+ 
     theme(plot.margin = unit(c(0,30,0,0), "pt"),
           plot.title = element_text(color = "grey15",
                                     ...
                                     face = "bold",
                                     size = 10)))  +
  (t_p + labs(title = "Time") + theme(plot.title = element_text(color = "grey15",
                                                                family = "Arial",
                                                                face = "bold",
                                                                size = 10))) +
  plot_layout(heights = c(1.25,.5, 1.25))
...
```{r, eval=FALSE, echo=TRUE, warning=FALSE, fig.height= 4, fig.width=11}
...
```{r, eval=FALSE, echo=TRUE, warning=FALSE, fig.height= 5, fig.width=14}
...
```{r, eval=FALSE, echo=TRUE, warning=FALSE, fig.height= 3, fig.width=9}
...
# Using outputs from 5 to summarize p_curve output
